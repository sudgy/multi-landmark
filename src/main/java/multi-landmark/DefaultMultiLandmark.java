...
public class DefaultMultiLandmark extends AbstractOp implements MultiLandmark {
    ...
    private int[] M_biggest_to; // An array used to determine which image is "biggest".
    ...
    private class ModelData {
        ...
        public ModelData(int i, int j) throws NotEnoughDataPointsException
        {
            ...
            double determinant = model_array[0] * model_array[3] - model_array[1] * model_array[2];
            if (determinant < 1) {
        ...
        public void try_transform(int index)
        {
            if (M_target == index) {
                ...
                ImageStack result = new ImageStack(P_images[M_target].getWidth(), P_images[M_target].getHeight());
                for (int i = 0; i < stack_size; ++i) {
                    if (stack_size > 1) {
                        // Note that the status bar will reset multiple times depending on how many ImagePluses you started with
                        P_status.showStatus(i + 1, stack_size, "Transforming " + P_images[M_source].getTitle());
                    }
                    ImageProcessor source = P_images[M_source].getStack().getProcessor(i + 1);
                    source.setInterpolationMethod(P_interpolation_method);
                    ImageProcessor target = M_processors[M_target].createProcessor(P_images[M_target].getWidth(), P_images[M_target].getHeight());
                    ...
                    result.addSlice(P_images[M_source].getStack().getSliceLabel(i + 1), target);
                }
                P_output[M_source] = new ImagePlus(P_images[M_source].getTitle() + " final", result);
            }
        }
        /*
         ...
         */
        private void put_pixel(ImageProcessor source, ImageProcessor target, double sx, double sy, int tx, int ty, final float[][] pixels)
        {
            if (sx >= 0 && (sx+0.5) < M_source_width && sy >= 0 && (sy+0.5) < M_source_height) {
                if (P_stop_interpolation) {
                    ...
                    for (int new_x = x_pos-1; new_x <= x_pos+1; ++new_x) {
                        for (int new_y = y_pos-1; new_y <= y_pos+1; ++new_y) {
                            ...
                            if (new_x < 0 || new_y < 0 || new_x >= M_source_width || new_y >= M_source_height) continue;
                            float new_value = pixels[new_x][new_y];
                            if (Math.abs(base_value - new_value) > P_discontinuity_threshold) {
                                target.putPixel(tx, ty, source.getPixel(x_pos, y_pos));
                                return;
                            }
                        }
                    }
